    ...
    <Layout>
      ...
      >
        <div className={styles.title}>
          ...
          <h4 className={universal.white}>
            Performer - Designer - Theatre Maker
          </h4>
        </div>
      </div>

      <ProjectList title="Current Work" posts={posts} />

      <CategorySelector page="Home" />
    </Layout>
  )
}
...
export const query = graphql`
  query HomePageQuery {
    ...
    currentWork: allMarkdownRemark(
      sort: { fields: [frontmatter___date], order: DESC }
      limit: 4
      filter: { frontmatter: { current_work: { eq: true } } }
    ) {
      edges {
        node {
          fields {
